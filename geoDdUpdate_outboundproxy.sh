...
#The curl commands throughout this script have the -k flag enabled, the outbound proxy I used has a self-signed cert
PROXYOUT=192.168.1.76:10443
...
function performAuthGetUrls {
    ...
    LOGINRESP=$(curl -x $PROXYOUT -vk 'https://api-u.f5.com/auth/pub/sso/login/user' -H 'Origin: https://login.f5.com' -H 'Content-Type: application/x-www-form-urlencoded' -H 'Referer: https://login.f5.com/' -H 'Accept-Language: en-US,en;q=0.9' --data-raw $RAWDATAF5 2>&1 | grep 'Set-Cookie')
    if [ -z "$LOGINRESP" ];
    then
    ...
    sleep 5
    EULARESP=$(curl -x $PROXYOUT -k -s -i $GEOLOCREG_URL -H "Cookie: $COOKIESSO; $COOKIESSOCOMP; $COOKIEUSR; $COOKIEF5SID"  2>&1 | grep 'Set-Cookie')
    if [ -z "$EULARESP" ];
    then
    ...
    GETHTML=$(curl -x $PROXYOUT -k -s -H "Cookie: $COOKIEJSESSIONID; $COOKIESSOCOMPL; $COOKIESSO; $COOKIEUSR; $COOKIEBIGIPDWNLS; $COOKIETS1; $COOKIETS2" "$GEOLOCREG_URL" 2>&1 | grep zip)
    if [ -z "$GETHTML" ];
    then
    ...
    DWNLDZIPMD5URL=$(curl -k -s -H "Cookie: $COOKIEJSESSIONID; $COOKIESSOCOMPL; $COOKIESSO; $COOKIEUSR; $COOKIEBIGIPDWNLS" "https://downloads.f5.com/esd/$ZIPMD5URL" | grep -B10 HTTPS: | grep -Po 'a href=".*?(?=")' | sed 's/a href="//g')
    if [ -z "$DWNLDZIPMD5URL" ];
    then
        echo "----- EXITING: Getting HTML to find download URL location failed"
        exit 1
...
function getGeodbZip {
    ...
    DWNLDZIP=$(curl -x $PROXYOUT -k -w "%{http_code}" --output $GEOLOCREG_AVAIL.zip --silent $DWNLDZIPURL)
    if [ $DWNLDZIP -eq 200 ]
    then
    ...
    DWNLDMD5=$(curl -x $PROXYOUT -k -w "%{http_code}" --output $GEOLOCREG_AVAIL.zip.md5 --silent $DWNLDZIPMD5URL)
    if [ $DWNLDMD5 -eq 200 ]
    then
